...
public class Board {
	...
	public static void drawBoard(Graphics g) {
		int boardX = (int)Hendow.SCREEN_SIZE.getWidth()/2 - WIDTH/2;
		int boardY = (int)Hendow.SCREEN_SIZE.getHeight()/2 - HEIGHT/2;
		g.drawRect(boardX, boardY, WIDTH, HEIGHT);
	
		int subwidth = WIDTH / instance.board[0].length;
		int subheight = HEIGHT / instance.board.length;
		for(int i = 0; i < instance.board.length; i++) {
			for(int j = 0; j < instance.board.length; j++) {
				int subboardX = boardX + j * subwidth;
				int subboardY = boardY + i * subheight;
	...
	public int calcProximity(int row, int col) {
		...
		for(int i = -1; i <=1; i++) {
			for(int j = -1; j <= 1; j++) {
				if(i == 0 && j == 0) continue;
				if(0 <= row + i && row + i <= board.length-1 && 
						0 <= col+j&&col + j <= board[0].length-1) {
					if(isBomb(row+i, col+j)) count++;
				}
			}
		}
		return count;
	...
	public void uncover(int row, int col) {
		...
		if(board[row][col] % 10 == 0) {
			for(int i = -1; i<=1; i++) {
				for(int j = -1; j <= 1; j++) {
					if(i == 0 && j == 0) continue;
					if(0 <= row+i && row+i <= board.length-1 && 0 <=col + j && col + j <= board[0].length-1) {
